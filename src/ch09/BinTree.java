...
public class BinTree<K,V> {
    ...
    public boolean remove(K key){
        ...
        Node<K,V> parent = null; // 스캔 중인 부모의 노드
        ...
        while(true){
            ...
            if(cond == 0) // 같으면
                ...
                if( cond < 0){ // 키 쪽이 작으면
                    ...
                    isLeftChild = false; // 오른쪽 자식으려 내려감
        ...
        if (p.left == null){ //p에 왼쪽 자식이 없음
             if(p == root)
                 ...
                 parent.left = p.right; // 부모의 왼쪽 포인터가 오른쪽 자식 가리킴
            else
            ...
            parent = p;
            Node<K,V> left = p.left; // 서브틀 ㅣ가운데 가장 큰 노드
            isLeftChild = true;
            while(left.right != null){ //가장큰 노드 left를 검색
                parent = left;
                left= left.right;
                isLeftChild = false;
            }
